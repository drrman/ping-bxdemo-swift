// CustomerConfig.swift
// Auto-generated by BXDemo Configurator
// Customer: Empire Consulting
// Generated: 2026-04-06T16:46:02.540Z

import SwiftUI

enum AuthMode {
    case davinci
    case oidcRedirect
}

struct ContentTile: Identifiable {
    let id = UUID()
    let title: String
    let subtitle: String
    let icon: String
    enum TileAction { case navigate, stepUp }
    let action: TileAction
}

struct CustomerConfig {
    let appName: String
    let tagline: String
    let primaryColor: Color
    let secondaryColor: Color
    let buttonColor: Color
    let buttonBgColor: Color
    let footerBgColor: Color
    let homeBannerColor: Color
    let logoAssetName: String
    let bannerAssetName: String
    let vertical: String
    let customerSlug: String
    let authMode: AuthMode
    let contentTiles: [ContentTile]
    let stepUpTitle: String
    let stepUpSubtitle: String
}

extension CustomerConfig {
    static let current = CustomerConfig(
        appName: "Empire Consulting",
        tagline: "Without a heart, it's just a machine.",
        primaryColor: Color(hex: "#304CB2"),
        secondaryColor: Color(hex: "#295eff"),
        buttonColor: Color(hex: "#FFFFFF"),
        buttonBgColor: Color(hex: "#304CB2"),
        footerBgColor: Color(hex: "#304CB2"),
        homeBannerColor: Color(hex: "#295eff"),
        logoAssetName: "empire-consulting-logo",
        bannerAssetName: "empire-consulting-banner",
        vertical: "technology",
        customerSlug: "empire-consulting",
        authMode: .davinci,
        contentTiles: [
        ContentTile(
            title: "My Consulting",
            subtitle: "View and manage your upcoming installs",
            icon: "house.fill",
            action: .navigate
        ),
        ContentTile(
            title: "Rapid Rewards",
            subtitle: "You have 24,500 points — redeem for your next service",
            icon: "star.fill",
            action: .navigate
        ),
        ContentTile(
            title: "Deals",
            subtitle: "Exclusive member offers available now",
            icon: "tag.fill",
            action: .navigate
        ),
        ContentTile(
            title: "Documents",
            subtitle: "Security",
            icon: "lock.shield.fill",
            action: .stepUp
        )
        ],
        stepUpTitle: "Verify Your Identity",
        stepUpSubtitle: "This section requires additional verification"
    )
}
